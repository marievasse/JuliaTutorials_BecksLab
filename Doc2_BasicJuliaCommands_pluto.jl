...
# ╔═╡ 425d6278-ca18-4f41-8dbe-4757a4c942a8
# second and fourth postion:
ar[[2,4]]

# ╔═╡ 9430bd45-40d0-4fd2-8ee1-1bb5577906ca
ar[2:4]
...
# ╔═╡ 778fea78-f196-4b40-a384-632fa442e2ca
nanvec = fill(NaN, 5, 5)
...
# write out a CSV:
CSV.write("data/myfishdata.csv", dat2)

# ╔═╡ d867329e-4ed6-11eb-0739-4dada7e2f3dc
# read in a CSV:
dat_in = CSV.read("data/myfishdata.csv", DataFrame)

# ╔═╡ d8695718-4ed6-11eb-2728-07d16a440e76
# alternatively, files can be exported as text files:
writedlm(join(["data/testing_", dat2[1,1], "_rates.txt"]), mat, "\t") # join does what it says on the tin - it joins variables into a single string

# ╔═╡ d87ba9fe-4ed6-11eb-1872-57f5931c928f
# or imported as text files:
dat_txt = readdlm(join(["data/testing_", dat2[1,1], "_rates.txt"]))
...
# ╔═╡ ce5a9a09-a83f-4e65-8def-0c3a4bc78801
# define a new value
x_new = 33.0 
...
# ╔═╡ b2001bd5-4bf5-4009-b428-7944c89bcfcf
global j=0 # set a counter in the global scope

# ╔═╡ ea034f78-53d4-4832-a6f7-b2e763cfa2f6
while(j<30) # justify a condition
    println(j) # prints j until j < 30
    j += 1 # count
end
...
# ╔═╡ 9ee28850-88c4-4157-b6ee-9451b84c6c42
collect(x4)
...
# global array:
B = zeros(1:10) #Should be zero(1:10)? or zeros(10) ?
