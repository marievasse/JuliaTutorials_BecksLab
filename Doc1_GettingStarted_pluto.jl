...
- Installing and loading packages using Julia's package manager (`Pkg`)
...
- navigate to the 'Marketplace' (5th symbol down in the activity bar - vertical panel on the lefthand side of the screen, called 'Extensions')
...
- name your script as your see fit but please remember to include the .jl file extension (e.g. JuliaTuto.jl)
...
Alternatively, you can also open a project in VS Code by right-clicking on your folder (in Finder, Windows file explorer or Linux nautilus) and selecting Open with -> Other -> VS Code. If you are a command-line addict, you can `cd` into the directory and type `code .` to open VS Code (this needs [to be activated for Mac](https://code.visualstudio.com/docs/setup/mac)).
...
The "Background and design" section provides useful details if you want to understand more about its originality compared to other languages and how it works. 

## Project and Manifest files 

We mentioned above that it is good practice to work within an environment specific to each project. The Julia package manager (`Pkg`) allows you to do that easily: *Unlike traditional package managers, which install and manage a single global set of packages, Pkg is designed around environments: independent sets of packages that can be local to an individual project or shared and selected by name* (text taken directly from [the documentation](https://julialang.github.io/Pkg.jl/v1/)).  
...
- check/click the 'Julia env:...' on the bottom of your screen, it should match your project name
...
Now that we are all set up, we are going to install a package, check the project's status and remove a package. As this might be your first time installing a package (e.g., Plots), dont be concerned if it takes a couple of minutes to run.
