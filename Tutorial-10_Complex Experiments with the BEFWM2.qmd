...
for alpha_ij in interspecific_vals
    ...
    # we specify E and F as before to re-enforce our understanding
    E = Environment(foodweb, K = 10)
    F = BioenergeticResponse(foodweb, h = 1)

    # we specify competition here
    # we fix intraspecific and variy interspecific
    Comp = ProducerCompetition(foodweb; αii = 1, αij = alpha_ij)

    # define parameters with extras
    params_comp = ModelParameters(foodweb, environment = E, functional_response = F, producer_competition = Comp)
    ...
    # collect data 
    fin_rich = foodweb_richness(out_comp, last = 30)
    fin_bio = total_biomass(out_comp, last = 30)
    stab = population_stability(out_comp, last = 30)
